...
class TrustRootTests(unittest.TestCase):
 def repo(self):
  ...
  (r/'.github/CODEOWNERS').write_text('\n'.join(f'/{rel} @TimSimmons3' for rel in MAN['trusted_paths'])+'\n')
  return r
 ...
 def test_codeowners_shadow_rule_without_project_owner_fails_migration(self):
  r=self.repo();b=self.commit(r,'base');co=r/'.github/CODEOWNERS';co.write_text(co.read_text()+'/scripts/validate_transition_metrics.py @Other\n');changed=['.github/CODEOWNERS'];record='docs/Releases/Assurance-Trust-Root-Migration-CODEOWNERS.json';reason='ownership change';rp=r/record;rp.parent.mkdir(parents=True,exist_ok=True);rp.write_text(json.dumps({'record_type':'SMT_ASSURANCE_TRUST_ROOT_MIGRATION','schema_version':'1.0','base_commit':b,'changed_trust_root_paths':changed,'owner_login':MAN['owner_login'],'rationale':reason,'rationale_sha256':hashlib.sha256(reason.encode()).hexdigest()},sort_keys=True));head=self.commit(r,'change');expected=t.expected_approval(5,head,record,changed);x=t.validate(r,b,MAN,'migration',5,[{'user':{'login':MAN['owner_login']},'body':expected}]);self.assertEqual('FAIL',x['status']);self.assertTrue(any('final applicable rule' in y for y in x['violations']),x)
 def test_codeowners_unsupported_pattern_fails_closed(self):
  r=self.repo();b=self.commit(r,'base');co=r/'.github/CODEOWNERS';co.write_text(co.read_text()+'/** @TimSimmons3\n');self.commit(r,'change');x=t.validate(r,b,MAN,'migration',5,[]);self.assertEqual('FAIL',x['status']);self.assertTrue(any('unsupported R1 pattern syntax' in y for y in x['violations']),x)
 def test_new_trusted_path_without_codeowners_coverage_fails(self):
  r=self.repo();b=self.commit(r,'base');cm=copy.deepcopy(MAN);new='scripts/new_trusted.py';cm['trusted_paths']=sorted(cm['trusted_paths']+[new]);(r/'config/assurance-trust-root-manifest.json').write_text(json.dumps(cm,sort_keys=True));(r/new).write_text('x\n');changed=sorted(['config/assurance-trust-root-manifest.json',new]);record='docs/Releases/Assurance-Trust-Root-Migration-NEW.json';reason='add trusted';rp=r/record;rp.parent.mkdir(parents=True,exist_ok=True);rp.write_text(json.dumps({'record_type':'SMT_ASSURANCE_TRUST_ROOT_MIGRATION','schema_version':'1.0','base_commit':b,'changed_trust_root_paths':changed,'owner_login':MAN['owner_login'],'rationale':reason,'rationale_sha256':hashlib.sha256(reason.encode()).hexdigest()},sort_keys=True));head=self.commit(r,'change');expected=t.expected_approval(5,head,record,changed);x=t.validate(r,b,MAN,'migration',5,[{'user':{'login':MAN['owner_login']},'body':expected}]);self.assertEqual('FAIL',x['status']);self.assertTrue(any('no applicable rule' in y for y in x['violations']),x)
 def test_historical_migration_record_unchanged_allows_unrelated_change(self):
  r=self.repo();old=r/'docs/Releases/Assurance-Trust-Root-Migration-OLD.json';old.parent.mkdir(parents=True,exist_ok=True);old.write_text('{}\n');b=self.commit(r,'base');(r/'note').write_text('x');self.commit(r,'change');x=t.validate(r,b,MAN,'ordinary',None,[]);self.assertEqual('PASS',x['status'],x['violations'])
 def test_historical_migration_record_deletion_fails(self):
  r=self.repo();old=r/'docs/Releases/Assurance-Trust-Root-Migration-OLD.json';old.parent.mkdir(parents=True,exist_ok=True);old.write_text('{}\n');b=self.commit(r,'base');old.unlink();self.commit(r,'delete');x=t.validate(r,b,MAN,'ordinary',None,[]);self.assertEqual('FAIL',x['status']);self.assertTrue(any('historical assurance trust-root migration record is immutable' in y for y in x['violations']),x)
 def test_historical_migration_record_rewrite_fails(self):
  r=self.repo();old=r/'docs/Releases/Assurance-Trust-Root-Migration-OLD.json';old.parent.mkdir(parents=True,exist_ok=True);old.write_text('{}\n');b=self.commit(r,'base');old.write_text('{"changed":true}\n');self.commit(r,'rewrite');x=t.validate(r,b,MAN,'ordinary',None,[]);self.assertEqual('FAIL',x['status']);self.assertTrue(any('historical assurance trust-root migration record is immutable' in y for y in x['violations']),x)
 def test_historical_migration_record_type_change_fails(self):
  r=self.repo();old=r/'docs/Releases/Assurance-Trust-Root-Migration-OLD.json';old.parent.mkdir(parents=True,exist_ok=True);old.write_text('{}\n');b=self.commit(r,'base');old.unlink();old.symlink_to('target');self.commit(r,'type');x=t.validate(r,b,MAN,'ordinary',None,[]);self.assertEqual('FAIL',x['status']);self.assertTrue(any('historical assurance trust-root migration record is immutable' in y for y in x['violations']),x)
